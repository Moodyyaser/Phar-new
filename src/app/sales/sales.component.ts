...
export class SalesComponent implements OnInit {
        ...
        this.tableService
            ...
            .subscribe(() => {
                ...
                localStorage.setItem(
                    "selllength_" + te,
                    this.table_length.toString()
                );
                localStorage.setItem(
                    "selldate_" + te,
                    this.date_index.toString()
                );
                ...
                for (
                    let i = this.table_length;
                    i < this.table_length_prev;
                    i += 1
                ) {
    ...
    load(num: number) {
        this.tableService.getElements().subscribe(() => {
            ...
            this.date_index = new Date(
                localStorage.getItem("selldate_" + te) || ""
            );
            this.buyer_index =
                localStorage.getItem("sellcompany_" + te) ||
                this.company_buyers[0];
            this.Table_array = [];
            for (let i = 0; i < this.table_length; i += 1) {
