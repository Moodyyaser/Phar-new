...
export class SalesComponent implements OnInit {
  ...
  save() {
    this.tableService
      .updateSales(this.Table_array[this.export_number])
      .subscribe(() => {
        this.saved = true;
        ...
        for (let i = 0; i < this.table_length; i += 1) {
          localStorage.setItem(
            'sellname' + i.toString() + '_' + te,
            this.Table_array[i].name,
          );
          localStorage.setItem(
            'selltablets' + i.toString() + '_' + te,
            (this.Table_array[i].tablets || 0).toString(),
          );
          localStorage.setItem(
            'sellcapsulent' + i.toString() + '_' + te,
            (this.Table_array[i].capsulent || 0).toString(),
          );
          localStorage.setItem(
            'sellprice' + i.toString() + '_' + te,
  ...
  load(num: number) {
    this.tableService.getElements().subscribe(() => {
      ...
      this.table_length = parseInt(
        localStorage.getItem('selllength_' + te) || '0',
      );
      this.date_index = new Date(localStorage.getItem('selldate_' + te) || '');
      this.buyer_index =
        localStorage.getItem('sellcompany_' + te) || this.company_buyers[0];
      this.Table_array = [];
      for (let i = 0; i < this.table_length; i += 1) {
        this.Table_array.push({
          id: i,
          name:
            localStorage.getItem('sellname' + i.toString() + '_' + te) || '',
          capsulent: parseFloat(
            localStorage.getItem('sellcapsulent' + i.toString() + '_' + te) ||
              '0',
          ),
          tablets: parseFloat(
            localStorage.getItem('selltablets' + i.toString() + '_' + te) ||
              '0',
          ),
          price: parseFloat(
            localStorage.getItem('sellprice' + i.toString() + '_' + te) || '0',
          ),
        });
      }
      this.table_length_prev = this.table_length;
  ...
  sortData(sort: Sort) {
    ...
    this.sortedData = data.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'tablets':
          return compare(a.tablets, b.tablets, isAsc);
        case 'price':
          return compare(a.price, b.price, isAsc);
        case 'total':
          return compare(
            a.price * a.tablets * a.capsulent,
            b.price * b.tablets * b.capsulent,
            isAsc,
          );
        default:
          return 0;
